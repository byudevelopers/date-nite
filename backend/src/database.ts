...
export async function createUser(user: { id: string; email: string; favorites?: string[] }) {
  const { data, error } = await supabase.from('users').insert([user]).single();
  if (error) throw error;
  return data;
}

export async function updateUser(id: string, updates: Partial<{ email: string; favorites: string[] }>) {
...
export async function getDate(id: string) {
  const { data, error } = await supabase.from('dates').select('*').eq('id', id).single();
  if (error) throw error;
  return data;
}
...
export async function updateDate(id: string, updates: any) {
  const { data, error } = await supabase.from('dates').update(updates).eq('id', id).single();
  if (error) throw error;
  return data;
}
...
export async function updateRating(id: string, updates: any) {
  const { data, error } = await supabase.from('ratings').update(updates).eq('id', id).single();
  if (error) throw error;
  return data;
}
